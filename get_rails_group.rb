...
curriculum_data.each do |curriculum|
   arr.push(curriculum[1])
end
hour = arr.uniq.sort
...
puts curriculum_data.group_by { |data| data[1] }.map { |hour, values| { hour: hour, count: values.count, contents: values.map { |v| v[0] } } }.sort_by { |ans| -(ans[:hour]) }